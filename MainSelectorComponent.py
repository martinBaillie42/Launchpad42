...
class MainSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, top_buttons, side_buttons, config_button, osd, control_surface, note_repeat):
        ...
        self._stepseq = StepSequencerComponent(self._matrix, self._side_buttons, self._nav_buttons,
                                               self._control_surface)
        self._stepseq.set_osd(self._osd)

        # User2 stepSequencer (Retro style)
        self._stepseq2 = StepSequencerComponent2(self._matrix, self._side_buttons, self._nav_buttons,
                                                 self._control_surface)
        self._stepseq2.set_osd(self._osd)

        # User1 Instrument controller (Scale)
        self._instrument_controller = InstrumentControllerComponent(self._matrix, self._side_buttons, self._nav_buttons,
                                                                    self._control_surface, self._note_repeat)
        ...
        self._device_controller = DeviceComponent(control_surface=self._control_surface, matrix=self._matrix,
                                                  side_buttons=self._side_buttons, top_buttons=self._nav_buttons)
        ...
        if self.is_enabled():
            ...
            if self._main_mode_index == 0:
                ...
                self._setup_sub_mode(
                    Settings.USER_MODES[(self._main_mode_index - 1) * 3 + self._sub_mode_list[self._main_mode_index]])
    ...
    def _setup_session(self, as_active, as_navigation_enabled):
        ...
        if as_active:  # Set up stop clip buttons and stop all clips button
            if self._session._stop_clip_buttons != None:
                ...
                self._side_buttons[self._session._num_scenes].set_on_off_values("Session.StopClip",
        ...
        if as_active:  # zoom
            ...
            self._zooming.set_nav_buttons(self._nav_buttons[0], self._nav_buttons[1], self._nav_buttons[2],
                                          self._nav_buttons[3])
    ...
    def _setup_user_mode(self, release_matrix=True, release_side_buttons=True, release_nav_buttons=True,
                         drum_rack_mode=True):
