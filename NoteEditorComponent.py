            ...
            if self._key_index_is_root_note[i]:
                for j in range(0, self.number_of_lines_per_note):
                    self._grid_back_buffer[0][
                        self.height - i * self.number_of_lines_per_note - j - 1] = "StepSequencer.NoteEditor.NoteMarker"
            ...
            if self._clip != None and self._note_cache != None:
                ...
                for note in self._note_cache:
                    ...
                    if (note_grid_x_position >= 0):
                        ...
                        if not note_muted and note_page == play_page and play_x_position == note_grid_x_position and (
                                    play_y_position == note_grid_y_position and not self.is_multinote or self.is_multinote and note_grid_y_offset == play_row) and self.song().is_playing and self._clip.is_playing:
        ...
        if self.is_enabled() and self._clip == None:
            ...
            if value != 0 or not is_momentary:  # if NOTE_ON or button is toggle
                ...
                if self.is_multinote:  # Calculate note pitch and time for notes
                    time = self.quantization * (self._page * self.width * self.number_of_lines_per_note + x + (
                ...
                for note in note_cache:
                    if pitch == note[0] and time == note[1]:
                        if self._is_velocity_shifted:
                            ...
                            note_cache.append([note[0], note[1], note[2], self.velocity_map[new_velocity_index],
                                               note[4]])  # (pitch, time, duration, velocity, mute state)
                        elif not self._is_mute_shifted:
                            note_cache.remove(note)
                        else:
                            # mute / un mute note.
                            note_cache.append([note[0], note[1], note[2], note[3],
                                               not note[4]])  # (pitch, time, duration, velocity, mute state)
                        break
                else:
                    note_cache.append([pitch, time, duration, velocity,
                                       self._is_mute_shifted])  # (pitch, time, duration, velocity, mute state)
                self._clip.select_all_notes()
        ...
        if self.is_enabled() and self._clip != None:
            ...
            for note in self._note_cache:
                if note[0] == pitch_to_mute:
                    ...
                    note_cache.remove(note)
                    note_cache.append(
                        [note_to_mute[0], note_to_mute[1], note_to_mute[2], note_to_mute[3], not note_to_mute[4]])
            if notes_changed > 0:
                self._clip.select_all_notes()
                self._clip.replace_selected_notes(tuple(note_cache))
                note_cache = self._clip.get_selected_notes()
    ...
    def _display_current_page(self):  # OK
        for i in range(0, self._height):
            if (self._page == self._current_page):
                self._grid_back_buffer[self._current_page % self.width][
                    i] = "StepSequencer.NoteEditor.CurrentPageMarkerPlay"
            else:
                self._grid_back_buffer[self._current_page % self.width][
                    i] = "StepSequencer.NoteEditor.CurrentPageMarker"
