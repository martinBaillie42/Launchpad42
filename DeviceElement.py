...
class DeviceElement(LiveDeviceComponent):
    __module__ = __name__
    __doc__ = ''

    def __init__(self, control_surface=None, name="device_component", is_enabled=False, matrix=None, side_buttons=None,
                 top_buttons=None):
        ...
        if self._matrix:
            ...
            for column in range(self._matrix.width()):
                slider = DeviceControllerStrip(tuple(
                    [self._matrix.get_button(column, (self._matrix.height() - 1 - row)) for row in
                     range(self._matrix.height())]), self._control_surface)
        ...
        if self.is_enabled():
            ...
            if self._device == None and self._matrix != None:
                for x in range(self._matrix.width()):
                    for y in range(self._matrix.height()):
                        if self._force:
                            self._matrix.get_button(x, y).set_on_off_values("DefaulyButton.Disabled",
            ...
            if value != 0:  # Button down
                ...
                if now - self._lock_button_press[index] > 0.4:  # long press
                    if self._locked_devices[index] == None:  # If lock is available
                        ...
                        if dev >= 0:  # Device already stored
                            if self._device != None:
                                self._control_surface.show_message(
                                    "*** WARNING *** '" + self.get_device_track_name(self._device) + " - " + str(
                                        self._device.name) + "' IS ALREADY IN LOCK: " + str(dev + 1))
                        else:  # New device is added
                            if self._device != None:
                                self._locked_devices[index] = self._device
                                if self._device != None:
                                    self._control_surface.show_message(
                                        " '" + self.get_device_track_name(self._device) + " - " + str(
                                            self._device.name) + "' STORED IN LOCK: " + str(index + 1))
                        ...
                        if self._locked_devices[index] != None:
                            self._control_surface.show_message(
                                "REMOVING '" + self.get_device_track_name(self._locked_devices[index]) + " - " + str(
                                    self._locked_devices[index].name) + "' FROM LOCK: " + str(index + 1))
                    ...
                    if self._locked_device_index == index:
                        if self._locked_devices[index] != None:
                            if self._locked_devices[index] != None:
                                self._control_surface.show_message("UNLOCKED FROM '" + self.get_device_track_name(
                                    self._locked_devices[index]) + " - " + str(
                                    self._locked_devices[index].name) + "' (" + str(index + 1) + ")")
                        self._locked_device_index = None
                        ...
                        if self._locked_devices[index] != None:
                            self._control_surface.show_message(
                                "LOCKED TO '" + self.get_device_track_name(self._locked_devices[index]) + " - " + str(
                                    self._locked_devices[index].name) + " (" + str(index + 1) + ")")
                        self.update()
            self.update_track_buttons()
            self.update_device_buttons()
    ...
    def update_device_buttons(self):
        ...
        if self.is_enabled():
            ...
            if (self._next_device_button != None):
                ...
                if (len(self.selected_track().devices) > 0 and self.selected_device_idx < len(
