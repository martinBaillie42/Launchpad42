                ...
                if self._quick_scale_root == 0:
                    ...
                    if y == 1 and x < 7 or y == 0 and x in [0, 1, 3, 4, 5]:
                        if y == 1:
                            ...
                            self._control_surface.show_message(
                                keys[root] + " " + str(self._scales._modus_names[selected_modus]))
                        if y == 0 and x < 6:
                        ...
                        if y == 1 and x == 7:  # nav circle of 5th right
                            ...
                            self._control_surface.show_message("circle of 5ths -> " + keys[selected_key] + " " + str(
                                self._scales._modus_names[selected_modus]) + " => " + keys[root] + " " + str(
                                self._scales._modus_names[selected_modus]))
                        ...
                        if y == 0 and x == 2:  # relative scale
                            if selected_modus == 0:
                                ...
                                root = CIRCLE_OF_FIFTHS[(self.tuple_idx(CIRCLE_OF_FIFTHS, selected_key) - 3 + 12) % 12]
                            self._control_surface.show_message(
                                "Relative scale : " + keys[root] + " " + str(self._scales._modus_names[selected_modus]))
                    ...
                    if (y == 0):
                        if x < 7 and self._quick_scales[x] != -1:
                            ...
                            if Settings.INSTRUMENT__SAVE_SCALE != None and Settings.INSTRUMENT__SAVE_SCALE == "clip":
                                self._scales.update_object_name(self._track_controller.selected_clip)
                            self._control_surface.show_message(
                                "mode : " + str(self._scales._modus_names[self._scales._modus]))
                            self.update()
                        if x == 7:
                            self.setup_quick_scale_mode()
                            self.update()
                    ...
                    if (y == 0):
                        if x == 0:
                            ...
                            self._control_surface.show_message(
                                "REPEATER is: " + str("ON" if self._note_repeat.is_enabled() else "OFF"))
                        elif x == 7:
                            self.setup_quick_scale_mode()
                ...
                for button, (x, y) in self._matrix.iterbuttons():
                    if button:
                        ...
                        if note < 128 and note >= 0:
                            if self._drum_group_device != None and self._drum_group_device.can_have_drum_pads and self._drum_group_device.has_drum_pads and \
                                    self._drum_group_device.drum_pads[note].chains:
