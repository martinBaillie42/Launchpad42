...
class ScaleComponent(ControlSurfaceComponent):
        ...
        if key >= 0 and key <= 11:
            ...
            if message:
                self._control_surface.show_message(
                    str("Selected Scale: " + KEY_NAMES[self._key]) + " " + str(self._modus_names[self._modus]))
            ...
            if not self.is_drumrack:
                ...
                if y == 2 and x == 7:
                    ...
                    self._control_surface.show_message("circle of 5ths -> " + keys[selected_key] + " " + str(
                        self._modus_names[selected_modus]) + " => " + keys[root] + " " + str(
                ...
                if y == 1 and x == 2:
                    if self._modus == 0:  # Ionian Mode (Major)
                        selected_modus = self._current_minor_mode
                        root = CIRCLE_OF_FIFTHS[(CIRCLE_OF_FIFTHS.index(
                            selected_key) + 3) % 12]  # Jump up 3 steps in 5th Circle equals jumping a third minor down
                    elif self._modus in [1, 13, 14]:  # Natural (Aeolian), Harmonic, Melodic Minor
                        self._current_minor_mode = selected_modus
                        selected_modus = 0
                        root = CIRCLE_OF_FIFTHS[(CIRCLE_OF_FIFTHS.index(
                            selected_key) - 3 + 12) % 12]  # Jump down 3 steps in 5th Circle equals jumping a third minor up
                    ...
                    self._control_surface.show_message(
                        "Relative scale : " + keys[root] + " " + str(self._modus_names[selected_modus]))
                    message = False
                if root != -1:
    ...
    def is_diatonic(self):
        ...
        self._mode == "diatonic" or self._mode == "diatonic_ns" or self._mode == "diatonic_chords")
